...
function Post({
  ...
  useEffect(() => {
    ...
    if (auth.currentUser) {
      onValue(
        ref(db, "users/" + auth.currentUser.uid + "/myComments"),
        async (snapshot) => {
          if (snapshot.exists()) {
            const myComments = Object.values(await snapshot.val());
  ...
  return (
    <>
      ...
      />
      <div className="p-4 w-2/12 min-w-[320px] shadow-2xl rounded-3xl overflow-hidden m-4 bg-orange-200 hover:scale-105 hover:transition-all hover:ease-in-out transition-all ease-in-out">
        <div className="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
          ...
          <img
            className="max-h-[10rem] min-h-[10rem] w-full object-contain object-center"
            src={
          ...
          <div className="p-4">
            ...
            <div className="flex items-center flex-wrap ">
              <Link
                href={"/" + postId}
                className="text-green-800 sm:mb-2 lg:mb-0"
              >
                <p className="inline-flex items-center m-0 border-solid border-2 border-slate-400 bg-amber-400 cursor-pointer rounded-lg">
                  Read Blog
                  <svg
